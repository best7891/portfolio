...
class CommuOtopshopSupabaseUser extends BaseAuthUser {
  CommuOtopshopSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> commuOtopshopSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = CommuOtopshopSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
